...
La version V0 tenia varias responsabilidades en la clase InformeAppServiceV0, esta preparaba los datos de entrada
registraba personas en RegistroPersonas, generaba el informe en formato Markdown, mostraba mensajes
y resultados por consola y realizaba búsquedas dentro del registro, esto viola SRP porque la clase 
puede cambiar.
...
- PreparadorDatos: Construye y devuelve la lista de elementos a procesar
- ServicioRegistro: Registra personas y permite busquedas
- ServicioInforme: Se encarga de generar el informe

3) ¿Cómo mejora el mantenimiento/testeo con el refactor?

RESPUESTA:

El código es más facil de mantener porque cada componente tiene una función clara y aislada. Además, facilita el testeo por separado ya que
cada una puede probarse de manera independiente sin depender de las demás.
...
La clase GeneradorInformeV0 porque utilizaba un when sobre FormatoInformeV0.
Cada vez que se quiera añadir un nuevo formato hay que modificar el código existente.
...
He usado polimorfismo mediante una interfaz, para que cada formato tenga su propia clase. Así el principal trabaja con la interfaz y no necesita modificarse.
...
Se haría creando una nueva clase que implemente la interfaz.
